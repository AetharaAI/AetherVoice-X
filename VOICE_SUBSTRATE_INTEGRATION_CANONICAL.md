...
* AetherPro standard inference contract:
LiteLLM is the inference gateway/router.
It exposes an OpenAI-compatible API contract.
Clients send model + messages + params to one base URL with one API key pattern.
Underlying providers/models can change without changing the client integration.
